...
!function () {
        ...
        function f(b) {
            ...
            if (j ? b = j(b, d) : g && (b = b.replace(/\n/g, function () {
                ...
                if (b = b.replace(/^=[=#]?|[\s;]*$/g, ""), e) {
                    var f = b.replace(/\s*\([^\)]+\)/, "");
                    n[f] || /^(include|print)$/.test(f) || (b = "(function(){try{ return $escape(" + b + ")}catch(err){return ''}})()")
                } else {
                    b = "$string(" + b + ")"
                }
                b = s[1] + b + s[2]
    ...
    var d = function (a, b, callback) {
        return "string" == typeof b ? q(b, {filename: a}) : g(a, b, callback)
    };
    ...
    var g = d.renderFile = function (a, b, callback) {
        var c = d.get(a, callback) || p({filename: a, name: "Render Error", message: "Template not found"});
        return b ? c(b) : c
    };
    d.get = function (a, callback) {
        var b;
        if (f[a]) {
            b = f[a]
        } else {
    ...
    e.parser = function (a) {
        ...
        switch (c) {
            ...
            case"each":
                ...
                "as" !== g && (f = "[]"), a = "$each(" + "(function(){try{ return " + f + " }catch(err){return [] }})()" + ",function(" + j + "){";
                break;
            case"/each":
                a = "});";
                break;
